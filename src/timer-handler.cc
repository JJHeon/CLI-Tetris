...
/* class TimerData  ===================================================================================== */
TimerData::TimerData(std::unique_ptr<bool> accessor_allive) : accessor_allive_(std::move(accessor_allive)) {}
TimerData::TimerData(TimerData& obj) : accessor_allive_(std::move(obj.accessor_allive_)), timer_(obj.timer_) {}
TimerData::TimerData(TimerData&& obj) : accessor_allive_(std::move(obj.accessor_allive_)), timer_(obj.timer_) {}

/* TimerAccessor Class ===================================================================================== */

TimerAccessor::TimerAccessor(int id, bool* is_running_address) : id_(id), is_running_(is_running_address), is_allive_(nullptr) {
...
TimerData& TimerHandler::FindTimerData(TimerAccessor& accessor) {
    const auto& itr = timer_list_.find(accessor.id_);
}
...
TimerAccessor&& TimerHandler::CreateTimer() {
    ...
    TimerData data(std::make_unique<bool>(true));
    key.is_allive_ = data.accessor_allive_.get();
